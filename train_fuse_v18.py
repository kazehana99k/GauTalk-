...
def render_motion_with_albedo(viewpoint_camera, pc, motion_net, pipe, bg_color,
    ...
    # R-DIAG-AU25 (2026-05-26): set TG_DIAG_ZERO_AU25=1 to zero the AU25 channel
    # (index -1 of 7-dim exp_feat) for ablation rendering. Measures actual
    # contribution of the AU25 channel learned during training.
    import os as _os_diag
    if int(_os_diag.environ.get('TG_DIAG_ZERO_AU25', '0')) == 1 and exp_feat.numel() >= 7:
        exp_feat = exp_feat.clone()
        exp_feat[..., -1] = 0.0
    ...
    # R-FACE-LIP-OPEN: when AU25 high, geometrically push upper-lip Gaussians UP and
    # lower-lip Gaussians DOWN to make room for mouth interior. This is what face
    # SHOULD do (but can't learn since AU25 not in exp_feat).
    # Set env TG_FACE_LIP_OPEN_AMP > 0 (e.g., 0.005) to enable.
    import os as _os
    _yield = float(_os.environ.get('TG_FACE_LIP_YIELD', '0.0'))
    _open_amp = float(_os.environ.get('TG_FACE_LIP_OPEN_AMP', '0.0'))
    # exp_feat is the 6-dim [AU01,04,05,06,07,45] — AU25 not in it. Read au_exp17 for AU25.
    _au17 = viewpoint_camera.talking_dict.get('au_exp17', None)
    if _au17 is not None and not torch.is_tensor(_au17):
        _au17 = torch.as_tensor(_au17)
    if abs(_open_amp) > 1e-9 and _au17 is not None and _au17.numel() >= 15:
        with torch.no_grad():
            _au25 = float(_au17.view(-1)[14].item())
            _xyz = pc.get_xyz
            _mask_path = _os.environ.get('TG_FACE_LIP_MASK_3D', 'data/macron/lip_mask_3d.pt')
            if not hasattr(render_motion_with_albedo, '_lip3d_cache_open'):
                render_motion_with_albedo._lip3d_cache_open = torch.load(_mask_path)['mask'].to(_xyz.device).squeeze(-1)
            _lip = render_motion_with_albedo._lip3d_cache_open  # [N] in {0,1}
            _y_split = float(_os.environ.get('TG_FACE_LIP_Y_SPLIT', '-0.0442'))  # lip median
            # Jaw-drop mode: TG_FACE_LIP_OPEN_UPPER_RATIO scales upper-lip motion vs lower.
            # 0.0 = pure mandible rotation (lower drops, upper static); 1.0 = symmetric.
            _upper_ratio = float(_os.environ.get('TG_FACE_LIP_OPEN_UPPER_RATIO', '1.0'))
            _y = _xyz[:, 1]
            _upper = (_lip > 0.5) & (_y > _y_split)
            _lower = (_lip > 0.5) & (_y <= _y_split)
            _dy = torch.zeros_like(_y)
            _dy[_upper] = _open_amp * _au25 * _upper_ratio
            _dy[_lower] = -_open_amp * _au25
        means3D = means3D.clone()
        means3D[:, 1] = means3D[:, 1] + _dy
        if not hasattr(render_motion_with_albedo, '_open_printed'):
            print(f'[R-FACE-LIP-OPEN] active: amp={_open_amp} au25={_au25:.2f} '
                  f'upper_ratio={_upper_ratio} '
                  f'upper count={int(_upper.sum())} lower count={int(_lower.sum())} '
                  f'max_dy_up={(_open_amp*_au25*_upper_ratio):.4f} '
                  f'max_dy_down={(-_open_amp*_au25):.4f}', flush=True)
            render_motion_with_albedo._open_printed = True
    if _yield > 0 and _au17 is not None and _au17.numel() >= 15:
        with torch.no_grad():
            _xyz = pc.get_xyz
            _au25 = float(_au17.view(-1)[14].item())
            _mask_path = _os.environ.get('TG_FACE_LIP_MASK_3D', '')
            if _mask_path:
                if not hasattr(render_motion_with_albedo, '_lip3d_cache'):
                    render_motion_with_albedo._lip3d_cache = torch.load(_mask_path)['mask'].to(_xyz.device)
                _lip_mask = render_motion_with_albedo._lip3d_cache  # [N,1] in {0,1}
            else:
                _y = _xyz[:, 1]
                _x = _xyz[:, 0]
                _y_center = float(_os.environ.get('TG_FACE_LIP_Y_CENTER', '0.04'))
                _y_width = float(_os.environ.get('TG_FACE_LIP_Y_WIDTH', '0.04'))
                _x_center = float(_os.environ.get('TG_FACE_LIP_X_CENTER', '0.0'))
                _x_width = float(_os.environ.get('TG_FACE_LIP_X_WIDTH', '1e9'))
                _ymask = torch.exp(-((_y - _y_center) / _y_width).pow(2))
                _xmask = torch.exp(-((_x - _x_center) / _x_width).pow(2))
                _lip_mask = (_ymask * _xmask).unsqueeze(-1)
            _yield_factor = (_yield * _au25 * _lip_mask).clamp(0, 0.95)
        opacity_before = opacity.mean().item()
        opacity = (opacity * (1.0 - _yield_factor)).clamp_min(1e-4)
        if not hasattr(render_motion_with_albedo, '_lip_yield_printed'):
            print(f'[R-FACE-LIP-YIELD] active: yield={_yield} au25={_au25:.2f} '
                  f'lip_mask.max={_lip_mask.max().item():.3f} '
                  f'lip_mask.sum={_lip_mask.sum().item():.0f} '
                  f'opacity_before_mean={opacity_before:.3f} '
                  f'opacity_after_mean={opacity.mean().item():.3f}', flush=True)
            render_motion_with_albedo._lip_yield_printed = True
...
def render_motion_mouth_v18(viewpoint_camera, pc, motion_net_mouth, pipe, bg_color,
                              cross_attn_residual=None, landmark=None, motion_amplifier=None,
                              albedo_residual=None, au25=None):
    """V18: render the mouth Gaussian set with an extra cross-attn residual on
    top of MouthMotionNetwork's d_xyz.

    R-MOUTH-AU: optionally pass au25 scalar to motion_net_mouth so its zero-init
    au_mouth_branch can produce an additive d_xyz residual. If au25 is None it
    will be auto-extracted from viewpoint_camera.talking_dict (au_exp17 index 14).
    """
    from diff_gaussian_rasterization import GaussianRasterizationSettings, GaussianRasterizer

    audio_feat = viewpoint_camera.talking_dict["auds"].cuda()
    if au25 is None:
        _ae = viewpoint_camera.talking_dict.get('au_exp17', None)
        if _ae is None:
            _ae = viewpoint_camera.talking_dict.get('au_exp', None)
        if _ae is not None:
            _ae_t = _ae if torch.is_tensor(_ae) else torch.as_tensor(_ae)
            if _ae_t.numel() >= 15:
                au25 = _ae_t.view(-1)[14].float().to(pc.get_xyz.device)
    motion_preds = motion_net_mouth(pc.get_xyz, audio_feat, landmark=landmark, au25=au25)
    ...
    # R3 (5/20) audio-amplitude motion amplifier
    if motion_amplifier is not None:
        amp = 0.7 + 0.6 * motion_amplifier   # ∈ [0.7, 1.3] for ma_norm ∈ [0,1]
        d_xyz = d_xyz * amp
        d_rot = d_rot * amp
        d_scale = d_scale * amp
    ...
    # D2 (2026-05-20): audio-conditioned per-Gaussian RGB residual on mouth.
...
def render_fuse_v18(viewpoint_cam, gaussians, motion_net,
    ...
    pkg_mouth = render_motion_mouth_v18(viewpoint_cam, gaussians_mouth, motion_net_mouth,
                                          ...
                                          landmark=landmark,
                                          motion_amplifier=motion_amplifier,
...
def training(dataset, opt, pipe, debug_from,
             ...
             # full-protocol iters
             init_ckpt="macrontest/macron_v17/chkpnt_fuse_v17_latest.pth",
             total_iters=5000,
             au_window_T=8,
             aperture_w=0.2,
             freeze_face=False,
    ...
    # V18: parallel cross-attn driver for the mouth Gaussians, using the
    # MouthMotionNetwork's own tri-plane hashgrid feature dim and audio dim.
    ...
    cross_attn_driver_mouth = GaussianCrossAttnDriver(
        ...
        audio_seq_len=8,
        audio_dim=audio_dim_mouth,
        au_dim=17,
        d_model=128,
        n_heads=4,
        n_au_tokens=8,
        residual_scale_xyz=1e-3,    # V18.1: lowered from 2e-3 to keep PSNR
        residual_scale_scale=1e-3,
        residual_scale_rot=1e-3,
    ).cuda()
    ...
    if v14_extras is not None:
        ...
        if 'cross_attn_driver_mouth' in v14_extras:
            cross_attn_driver_mouth.load_state_dict(
                v14_extras['cross_attn_driver_mouth'], strict=False)
            print('[v18] loaded prior cross_attn_driver_mouth')
    ...
    for iteration in range(first_iter, total_iters + 1):
        ...
        with torch.no_grad():
            ...
            if iteration in checkpoint_iterations:
                ...
                if aperture_geom_w > 0:
                    _tag = "v26"
                elif arc_w > 0 or dino_w > 0:
                    _tag = "v25"
                elif freeze_face:
                    _tag = "v20"
                elif pixel_start_iter > 0:
                    _tag = "v24"
                elif detail_w > 0:
                    _tag = "v22"
                else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--init_ckpt", type=str,
                        default="macrontest/macron_v17/chkpnt_fuse_v17_latest.pth")
    parser.add_argument("--total_iters", type=int, default=5000)  # finetune from V17
