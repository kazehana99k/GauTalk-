...
> **本ドキュメントの対象範囲** — 以下は **描画スタック（3D Gaussian レンダラと表情・口形の駆動経路）**
> の設計です。頭部動作の検索・再生（ブロック分割、動作メモリ、切替ゲート、動作選択）は
> 本リポジトリの外にあり、論文公開に合わせて別途ドキュメント化します。
> システム全体の構成は [README.md](README.md#system-overview) を参照してください。
